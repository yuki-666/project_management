...
def mine():
    ...
    if request_data['career'] == config.career_project_manager:
        data = project.get_project(uid=request_data['uid'], include_reject=True)
    else:
        data = project.get_project_include_work_time(request_data['uid'])
...
def modify_show():
    ...
    data = project.get_project(project_id=request_data['id'], detail=True)
